# function for calculating fraction sizes (modified from S. Stoner)

FractionSizes <- function(csv.dir, temps_in, perC = 2, reqmgC = 0.5, dilution = 2){
  ...
  csvlist <- list.files(
    ...
    pattern = "*.csv",
    full.names = TRUE)
  csvlist <- csvlist[which(grepl("smooth", csvlist))]
  
  for (i in 1:length(csvlist)) {
    assign(csvlist[i], read.csv(csvlist[i]))
  }
