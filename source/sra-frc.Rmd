...
# plot inc v LF
# plot profiles by year and fraction type
inc.plot.fx <- function(dens.inc.df, frc, Year) {
...
```{r plot-inc-dens}
inc.plot.fx(dens.inc.min.w, "fPOM", "2019") 
inc.plot.fx(dens.inc.min.w, "oPOM")
inc.plot.fx(dens.inc.min.w, "minC", "2019")
inc.plot.fx(dens.inc.min.w, "minC", "2001")
```
...
```{r plot-tml-c-frc}
# bind_rows(
#   lapply(cfrc.ls, function(x) {
#     t0 <- x[1, ]
#     t0$temp_u <- 0
#     n_new <- rbind(t0, x)
#     n_new$c_pct <- c(x$c_pct, x$c_pct[nrow(x)])
#     return(n_new)
#   }), .id = "PMeco") %>%
#   ggplot(., aes(temp_u, c_pct, color = PMeco)) +
#   geom_step() +
#     scale_color_manual(name = NULL,
#                        values = c("ANwf" = andesite,
#                                   "BSwf" = basalt,
#                                   "GRwf" = granite),
#                        labels = c("ANwf" = "andesite, cool",
#                                   "BSwf" = "basalt, cool",
#                                   "GRwf" = "granite, cool")) +
#   ylab("% C") +
#   xlab("Temperature") +
...
```{r c-wtd-tml-14c}
# cfrc.ls2 <- lapply(seq_along(cfrc.ls), function(i) {
#   df <- cfrc.ls[[i]][2:nrow(cfrc.ls[[i]]), ]
#   df %>%
#     mutate(c_pct = c_pct * 10^-2,
#            thml_frc = seq(1, nrow(.)),
#            PMeco = names(cfrc.ls)[i],
#            PM = substr(PMeco, 1, 2),
#            ECO = substr(PMeco, 3, 4))
# })
# frc.14c.wtd.df <- merge(frc.14c.df, bind_rows(cfrc.ls2)) %>%
#   mutate(d14c_wtd = d14c * c_pct)
# 
# # step-plot
# frc.14c.wtd.df %>%
#   filter(thml_frc == 5) %>%
#   mutate(thml_frc = 6) %>%
#   rbind(frc.14c.wtd.df, .) %>%
#   ggplot(., aes(thml_frc, d14c_wtd, color = PM)) +
#   geom_hline(yintercept = 0, linetype = "dotted") +
#   geom_step() +
#   scale_color_manual(name = NULL,
#                      values = c("AN" = andesite,
...
#                                 "GR" = "granite")) +
#   scale_x_continuous(breaks = seq(1.5, 5.5), labels = as.character(seq(1, 5))) +
#   ylab(expression('C weighted '*Delta*''^14*'C (‰)')) +
#   xlab("Thermal fraction") +
#   theme_bw() +
#   theme(panel.grid = element_blank())
# 
# # cumulative C plots
# frc.14c.wtd.cmtv.df <- bind_rows(
#   lapply(split(frc.14c.wtd.df, frc.14c.wtd.df$PMeco), function(x) {
#     # x  <- x[order(x$d14c), ] # to order by d14c
#     for (i in 1:nrow(x)) {
#       if (i == 1) {
#         x$cmtv_c[i] <- x$c_pct[i] * 100
#         x$xmin[i] <- 0
#       } else {
#         x$cmtv_c[i] <- x$c_pct[i] * 100 + x$cmtv_c[i - 1]
#         x$xmin[i] <- x$cmtv_c[i - 1]
#       }
#     }
#     x$syn_14c <- sum(x$d14c_wtd)
#     return(x)
#   })
# )
# 
# # label facets
# f_labs <- data.frame(cmtv_c = 1, d14c = -145, 
#                      PM = c("AN", "BS", "GR"),
#                      minC_14c = round(unique(frc.14c.wtd.cmtv.df$syn_14c), 0))
# f_labs$label <- paste0(letters[1:3], ") ", 
#                        c("andesite = ", "basalt = ", "granite = "), 
#                       f_labs$minC_14c, "‰")
#                   
# 
# # col-plot
# frc.14c.wtd.cmtv.df %>%
#   # mutate(norm_14c = d14c - syn_14c) %>%
#   ggplot(., aes(cmtv_c, d14c)) +
#   geom_hline(yintercept = 0) +
#   geom_rect(aes(xmin = xmin, xmax = cmtv_c, 
#                 ymin = 0, ymax = d14c,
#                 fill = PM, alpha = temp_u), color = "black", size = .3) +
...
#                                 "BS" = "basalt",
#                                 "GR" = "granite")) +
#   scale_alpha_continuous(name = "Temperature (°C)",
#                          breaks = c(250, 350, 500, 800),
#                          limits = c(150, 800), range = c(0, 1)) +
#   scale_x_continuous(limits = c(0, 100), expand = c(0.01, 0.01)) +
#   facet_grid(rows = vars(PM)) +
#   ylab(expression(Delta*''^14*'C (‰)')) +
#   xlab("Percent of total mineral-associated C") +
#   theme_bw() +
#   theme(panel.grid = element_blank(),
...
# # as steps
# bind_rows(
#   lapply(split(frc.14c.wtd.cmtv.df, frc.14c.wtd.cmtv.df$PMeco), function(x) {
#     x_end <- x[nrow(x), ]
#     x_end$xmin <- 100
#     rbind(x, x_end)
#   })) %>%
#   ggplot(., aes(xmin, d14c, color = PM)) +
#   geom_hline(yintercept = 0) +
#   geom_step(size = 1) +
#   scale_color_manual(name = "Parent material",
#                      values = c("AN" = andesite,
